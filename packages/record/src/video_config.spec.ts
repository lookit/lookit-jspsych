...
let devicesObj: {cam1: MediaDeviceInfo, cam2: MediaDeviceInfo, mic1: MediaDeviceInfo, mic2: MediaDeviceInfo};
let returnedDeviceLists: {cameras: MediaDeviceInfo[], mics: MediaDeviceInfo[]};
...
beforeEach(() => {
  ...
  returnedDeviceLists = {cameras: [devicesObj.cam1, devicesObj.cam2], mics: [devicesObj.mic1, devicesObj.mic2]};
});
...
test("Video config trial method sets up trial", () => {
  const trial_info = { troubleshooting_intro: "" } as unknown as VideoConsentTrialType;

  // Set up mocks for upstream functions called in trial method.
  const addHtmlMock = jest.spyOn(video_config, "addHtmlContent").mockImplementation(jest.fn());
  const addEventListenerslMock = jest.spyOn(video_config, "addEventListeners").mockImplementation(jest.fn());
  const setupRecorderMock = jest.spyOn(video_config, "setupRecorder").mockImplementation(jest.fn());
  ...
  expect(global.navigator.mediaDevices.ondevicechange).toBe(video_config["onDeviceChange"]);
...
test("Video config addHtmlContent loads template with custom troubleshooting text", () => {
  ...
  let rendered_trial_html = Mustache.render(videoConfig, html_params_custom_intro);
  // Remove new lines, indents (tabs or spaces), and empty HTML property values.
  rendered_trial_html = rendered_trial_html.replace(
    /(\r\n|\n|\r|\t| {4})/gm,
    "",
...
test("Video config add event listeners", async () => {
  ...
  const nextBtnClickMock = jest.spyOn(video_config, "nextButtonClick").mockImplementation(jest.fn());
  const reloadBtnClickMock = jest.spyOn(video_config, "reloadButtonClick").mockImplementation(jest.fn()); 
...
test("Video config enable next", () => {
  ...
  expect(next_button_el.classList.contains(video_config["step_complete_class"])).toBe(false);
  expect(next_button_el.disabled).toBe(true);

  // Calling with 'true' enables the button and adds the class.
  video_config["enable_next"](true);
  expect(next_button_el.classList.contains(video_config["step_complete_class"])).toBe(true);
...
test("Video config update instructions", () => {
  ...
  const step1_span = video_config["display_el"]?.querySelector(`#${step1_id}-span`) as HTMLSpanElement;
  const step1_para = video_config["display_el"]?.querySelector(`#${step1_id}-paragraph`) as HTMLParagraphElement;
  const step2_span = video_config["display_el"]?.querySelector(`#${step2_id}-span`) as HTMLSpanElement;
  const step2_para = video_config["display_el"]?.querySelector(`#${step2_id}-paragraph`) as HTMLParagraphElement;
  const step3_span = video_config["display_el"]?.querySelector(`#${step3_id}-span`) as HTMLSpanElement;
  const step3_para = video_config["display_el"]?.querySelector(`#${step3_id}-paragraph`) as HTMLParagraphElement;
...
test("Video config reload button click", async () => {
  ...
  const updateInstructionskMock = jest.spyOn(video_config, "updateInstructions").mockImplementation(jest.fn());
  const destroyMock = jest.spyOn(video_config, "destroyRecorder").mockImplementation(jest.fn()); 
  const setupRecorderMock = jest.spyOn(video_config, "setupRecorder").mockImplementation(jest.fn()); 
...
test("Video config updateDeviceSelection", () => {
  ...
  video_config["updateDeviceSelection"]({cameras: [devicesObj.cam1], mics: [devicesObj.mic1]});
  expect(mic_selection_el.innerHTML).toContain(`<option value="${devicesObj.mic1.deviceId}"></option>`);
  expect(cam_selection_el.innerHTML).toContain(`<option value="${devicesObj.cam1.deviceId}"></option>`);

  // Also clears old options in select elements that are no longer in the devices lists.
  video_config["updateDeviceSelection"]({cameras: [devicesObj.cam2], mics: [devicesObj.mic2]});
  expect(mic_selection_el.innerHTML).not.toContain(`<option value="${devicesObj.mic1.deviceId}"></option>`);
  expect(cam_selection_el.innerHTML).not.toContain(`<option value="${devicesObj.cam1.deviceId}"></option>`);
  expect(mic_selection_el.innerHTML).toContain(`<option value="${devicesObj.mic2.deviceId}"></option>`);
  expect(cam_selection_el.innerHTML).toContain(`<option value="${devicesObj.cam2.deviceId}"></option>`);
  ...
  video_config["updateDeviceSelection"]({cameras: [devicesObj.cam1, devicesObj.cam2], mics: [devicesObj.mic1, devicesObj.mic2]});
  expect(mic_selection_el.innerHTML).toContain(`<option value="${devicesObj.mic1.deviceId}"></option>`);
  expect(cam_selection_el.innerHTML).toContain(`<option value="${devicesObj.cam1.deviceId}"></option>`);
  expect(mic_selection_el.innerHTML).toContain(`<option value="${devicesObj.mic2.deviceId}"></option>`);
  expect(cam_selection_el.innerHTML).toContain(`<option value="${devicesObj.cam2.deviceId}"></option>`);
  ...
  expect(mic_selection_el.innerHTML).toContain(`<option value="${devicesObj.mic1.deviceId}"></option>`);
  expect(cam_selection_el.innerHTML).toContain(`<option value="${devicesObj.cam1.deviceId}"></option>`);
  expect(mic_selection_el.innerHTML).not.toContain(`<option value="${devicesObj.mic2.deviceId}"></option>`);
  expect(cam_selection_el.innerHTML).not.toContain(`<option value="${devicesObj.cam2.deviceId}"></option>`);
...
test("Video config end trial", () => {
  ...
  expect(jsPsych.finishTrial).toHaveBeenCalledWith({rt: response.rt, camId, micId});
});

test("Video config setupRecorder", async () => {
  // Mock the upstream function calls.
  const updateInstructionsMock = jest.spyOn(video_config, "updateInstructions").mockImplementation(jest.fn());
  const updateErrorsMock = jest.spyOn(video_config, "updateErrors").mockImplementation(jest.fn());
  const requestPermissionMock = jest
    .spyOn(Recorder.prototype, "requestPermission")
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    .mockImplementation((constraints: MediaStreamConstraints) => Promise.resolve(jsPsych.pluginAPI.getCameraRecorder().stream));
  const getDeviceListsMock = jest
    .spyOn(Recorder.prototype, "getDeviceLists")
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    .mockImplementation((include_audio?: boolean | undefined, include_camera?:  boolean | undefined) => {
      return Promise.resolve(returnedDeviceLists)
    }); 
  const updateDeviceSelectionMock = jest.spyOn(video_config, "updateDeviceSelection").mockImplementation(jest.fn());
  const setDevicesMock = jest.spyOn(video_config, "setDevices").mockImplementation(jest.fn()); 
  const runStreamChecksMock = jest.spyOn(video_config, "runStreamChecks").mockImplementation(jest.fn()); 
  ...
  expect(updateErrorsMock.mock.calls[0]).toStrictEqual([video_config["waiting_for_access_msg"]]);
  expect(updateErrorsMock.mock.calls[1]).toStrictEqual([""]);
  // Needs to request permissions before it can get device lists.
  expect(requestPermissionMock).toHaveBeenCalledWith({video: true, audio: true});
  // Gets device lists.
...
test("Video config setDevices", async () => {
  ...
  const enableNextMock = jest.spyOn(video_config, "enable_next").mockImplementation(jest.fn());
  const updateInstructionsMock = jest.spyOn(video_config, "updateInstructions").mockImplementation(jest.fn());
  const requestPermissionMock = jest
    .spyOn(Recorder.prototype, "requestPermission")
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    .mockImplementation((constraints: MediaStreamConstraints) => Promise.resolve(jsPsych.pluginAPI.getCameraRecorder().stream));
  const initializeRecorderMock = jest
    .spyOn(Recorder.prototype, "intializeRecorder")
  ...
  expect(requestPermissionMock).toHaveBeenCalledWith({video: { deviceId: devicesObj.cam2.deviceId }, audio: { deviceId: devicesObj.mic2.deviceId }});
  ...
  expect(initializeRecorderMock).toHaveBeenCalledWith(jsPsych.pluginAPI.getCameraRecorder().stream);
  ...
  expect(requestPermissionMock).toHaveBeenCalledWith({video: { deviceId: devicesObj.cam1.deviceId }, audio: { deviceId: devicesObj.mic1.deviceId }});
});
...
test("Video config runStreamChecks throws NoStreamAccess", () => {
  ...
  expect(async () => await video_config["runStreamChecks"]()).rejects.toThrow(NoStreamError);
});
...
test("Video config runStreamChecks throws Mic Check error", async () => {
  ...
  await expect(video_config["runStreamChecks"]()).rejects.toThrow("Mic check problem");

  // If the recorder mic check throws an error, then the plugin should update the error message and throw the error.
  expect(updateErrorsMock).toHaveBeenCalledTimes(2);
  expect(updateErrorsMock.mock.calls[0]).toStrictEqual([video_config["checking_mic_msg"]]);
  expect(updateErrorsMock.mock.calls[1]).toStrictEqual([video_config["setup_problem_msg"]]);
});
...
test("Video config runStreamChecks", async () => {
  ...
  // Setup upstream mocks.
  const insertWebcamMock = jest.spyOn(Recorder.prototype, "insertWebcamFeed").mockImplementation(jest.fn());
  const updateInstructionsMock = jest.spyOn(video_config, "updateInstructions").mockImplementation(jest.fn());
  const updateErrorsMock = jest.spyOn(video_config, "updateErrors").mockImplementation(jest.fn());
  const checkMicMock = jest.spyOn(Recorder.prototype, "checkMic").mockImplementation(jest.fn(() => Promise.resolve()));
...
test("Video config device selection element on change event listener", async () => {
  ...
  const setDevicesMock = jest.spyOn(video_config, "setDevices").mockImplementation(jest.fn(() => Promise.resolve())); 
  const runStreamChecksMock = jest.spyOn(video_config, "runStreamChecks").mockImplementation(jest.fn(() => Promise.resolve())); 
  ...
  expect(runStreamChecksMock.mock.calls).toHaveLength(device_selection_els.length);
});

test("Video config next button click", async () => {
  video_config["start_time"] = 1;
  const perfNowMock = jest.spyOn(performance, "now").mockImplementation(jest.fn().mockReturnValue(5));
  const destroyMock =  jest.spyOn(video_config, "destroyRecorder").mockImplementation(jest.fn().mockReturnValue(Promise.resolve()));
  const endMock =  jest.spyOn(video_config, "endTrial").mockImplementation(jest.fn());
