...
/**
 * Collect all API call promises into an array to be checked with finish(). Once
 * a promise settles (fulfills or rejects), it is removed from the array so that
 * a past failure can't cause future, unrelated finish() calls to reject. The
 * promise itself is still returned as-is, so callers can handle its
 * resolution/rejection normally.
 *
...
const deposit = <T extends ApiPromise>(promise: T) => {
  ...
  Promise.resolve(promise)
    ...
    // The `.finally()` chain produces its own promise that mirrors any
    // rejection from `promise`. Swallow it here so it doesn't surface as an
    // unhandled rejection; the original `promise` (returned below) still
    // carries the rejection for the caller to handle.
    .catch(() => {});
  return promise;
};
